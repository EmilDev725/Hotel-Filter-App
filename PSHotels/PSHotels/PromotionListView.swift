...
class PromotionListView: PSUIView {
    ...
    override func initVariables() {
        hotelViewModel.nibName = "PromotionListView"
    ...
    @objc private func refresh() {
        ...
        hotelViewModel.promotionHotelListOffset = 0
        hotelViewModel.getPromotionHotelList(loginUserId: loginUserId)
    ...
    override func initData() {
        ...
        hotelViewModel.promotionHotelLiveData.bind{ [weak self] in
            ...
            if let newsListResourse : Resourse<[Hotel]> = $0 {
                ...
                if newsListResourse.status == Status.SUCCESS
                    ...
                    if let data = self?.hotelViewModel.promotionHotelLiveData.value?.data {
                        if data.count <= 0 {
                            self?.hotelListCollectionView?.reloadData()
...
extension PromotionListView : UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let data = hotelViewModel.promotionHotelLiveData.value?.data {
...
extension PromotionListView : UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
extension PromotionListView : PinterestLayoutDelegate {
    func collectionView(_ collectionView:UICollectionView, heightForPhotoAtIndexPath indexPath:IndexPath , withWidth width:CGFloat) -> CGFloat {
        
        if let data : Hotel = self.hotelViewModel.promotionHotelLiveData.value?.data![indexPath.row] {
...
extension PromotionListView : HotelViewDelegate {
    func refreshHotelData() {
        hotelViewModel.promotionHotelListOffset = 0
        hotelViewModel.getPromotionHotelList(loginUserId: loginUserId, isLoadFromServer: false)
