...
class RecommendedListView: PSUIView {
    ...
    override func initVariables() {
        hotelViewModel.nibName = "RecommendedListView"
    ...
    @objc private func refresh() {
        ...
        hotelViewModel.recommendedHotelListOffset = 0
        hotelViewModel.getRecommendedHotelList(loginUserId: loginUserId)
    ...
    override func initData() {
        ...
        hotelViewModel.recommendedHotelLiveData.bind{ [weak self] in
            ...
            if let newsListResourse : Resourse<[Hotel]> = $0 {
                ...
                if newsListResourse.status == Status.SUCCESS
                    ...
                    if let data = self?.hotelViewModel.recommendedHotelLiveData.value?.data {
                        if data.count <= 0 {
                            self?.hotelListCollectionView?.reloadData()
...
extension RecommendedListView : UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let data = hotelViewModel.recommendedHotelLiveData.value?.data {
...
extension RecommendedListView : UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
extension RecommendedListView : PinterestLayoutDelegate {
    func collectionView(_ collectionView:UICollectionView, heightForPhotoAtIndexPath indexPath:IndexPath , withWidth width:CGFloat) -> CGFloat {
        
        if let data : Hotel = self.hotelViewModel.recommendedHotelLiveData.value?.data![indexPath.row] {
...
extension RecommendedListView : HotelViewDelegate {
    func refreshHotelData() {
        hotelViewModel.recommendedHotelListOffset = 0
        hotelViewModel.getRecommendedHotelList(loginUserId: loginUserId, isLoadFromServer: false)
    }
}
