...
class HotelDetailView: PSUIView {
    ...
    @IBOutlet weak var youtubeCoverView: UIView!
    ...
    func bindHotelData(hotel: Hotel) {
        ...
        //Youtebe
        if (hotel.youtubevideo != "") {
            let youtubeURL: URL = URL(string: hotel.youtubevideo)!
            youtubePlayerView.loadVideoURL(youtubeURL)
        }else {
            youtubeCoverView.isHidden = true
        }
...
extension HotelDetailView : YouTubePlayerDelegate {
