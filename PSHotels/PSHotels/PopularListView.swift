...
class PopularListView: PSUIView {
    ...
    override func initVariables() {
        hotelViewModel.nibName = "PopularListView"
    ...
    override func initUIViewAndActions() {
        ...
        uiRefresher?.addTarget(self, action: #selector(refresh), for: UIControlEvents.valueChanged)
    ...
    @objc private func refresh() {
        ...
        hotelViewModel.popularHotelListOffset = 0
        hotelViewModel.getPopularHotelList(loginUserId: loginUserId)
    ...
    override func initData() {
        ...
        hotelViewModel.popularHotelLiveData.bind{ [weak self] in
            ...
            if let newsListResourse : Resourse<[Hotel]> = $0 {
                ...
                if newsListResourse.status == Status.SUCCESS
                    ...
                    if let data = self?.hotelViewModel.popularHotelLiveData.value?.data {
                        if data.count <= 0 {
                            self?.hotelListCollectionView?.reloadData()
...
extension PopularListView : UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let data = hotelViewModel.popularHotelLiveData.value?.data {
...
extension PopularListView : UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
extension PopularListView : PinterestLayoutDelegate {
    func collectionView(_ collectionView:UICollectionView, heightForPhotoAtIndexPath indexPath:IndexPath , withWidth width:CGFloat) -> CGFloat {
        
        if let data : Hotel = self.hotelViewModel.popularHotelLiveData.value?.data![indexPath.row] {
            ...
            //return topPaddingHeight + bottomPaddingHeight + betweenPaddingHeight + titleHeight + dateHeight + categoryHeight
...
extension PopularListView : HotelViewDelegate {
    func refreshHotelData() {
        hotelViewModel.popularHotelListOffset = 0
        hotelViewModel.getPopularHotelList(loginUserId: loginUserId, isLoadFromServer: false)
    }
}
